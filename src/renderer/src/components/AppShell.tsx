...
export function AppShell({
  ...
  // Phase 26.2 D-02 — auto-redirect: if the Unused tab is active and
  // orphanedFiles clears (e.g. resample removes all orphans), redirect back
  // to 'global' to avoid a stranded active-tab state. Watches the primitive
  // `.length` (not the array reference) to avoid infinite re-render loops
  // per T-26.2-03 mitigation in the threat model.
  useEffect(() => {
      ...
      <main className="flex-1 overflow-auto [scrollbar-gutter:stable]">
        ...
        */}
        ...
        {/* Phase 26.2 D-01/D-03 — Unused tab gate; query threaded from AppShell.
            Previously always-rendered (Phase 24 PANEL-02); now tab-gated so orphaned
            files have a dedicated panel surface without cluttering the Global viewport.
            The panel's own return null guard provides additional safety when empty. */}
        {activeTab === 'unused' && (
          <UnusedAssetsPanel
