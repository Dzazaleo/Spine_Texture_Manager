...
export function buildExportPlan(
  ...
  // not attachment-visibility vs sampler). Plan 02 will wire the new
  // exclusion surface.
  //
  ...
  // peakScale is always relative to canonical dims; using sourceW (= actualSourceW,
  // 670 for SQUARE) as the multiplier base gave ceil(670 × 0.670) = 449 instead of
  // ceil(1000 × 0.670) = 670 = actualSourceW → wrongly treated as a resize row.
  // Fallback to sourceW for legacy synthetic test data that omits canonicalW.
