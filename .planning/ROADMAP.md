...
- 🚧 **v1.2 (in progress)** — 7-phase expansion (Phases 13.1, 16, 18–22) authored 2026-04-30. Closes two macOS regressions + one host-blocked carry-forward from v1.1.x (13.1 + 16 + 18); refines the UI based on tester feedback (19); adds the Documentation Builder feature filling the `.stmproj` v1 reserved `documentation: object` slot from D-148 (20); lands the two long-dormant SEEDs — atlas-less mode (21) and dims-badge override-cap (22; depends on 21). 26 REQs across UAT, UPDFIX, QUIT, UI, DOC, LOAD, DIMS categories — UPDFIX-06 closed-by-test on 2026-04-30 (regression test 14-l in `tests/renderer/app-update-subscriptions.spec.tsx`; Phase 14 lift commit 802a76e already fixes the wiring), so Phase 17 was skipped. Apple Developer ID code-signing + Sentry crash reporting declined for v1.2 (revisit at v1.3); manual-download UX (Phase 16) is the locked v1.2 answer for macOS auto-update.
...
- [ ] **Phase 18: App quit broken — Cmd+Q + AppleScript do not terminate** — Renderer-side IPC subscription lift mirroring Phase 14 commit `802a76e`: move `onCheckDirtyBeforeQuit` from `AppShell.tsx` (mounted only on `loaded` / `projectLoaded`) to `App.tsx`'s top-level useEffect (always-mounted root) so the listener survives `idle` / `error` / `projectLoadFailed` AppStates. Bridge dirty signal + SaveQuitDialog dispatch back through a new `dirtyCheckRef`. No main-process or preload changes. Closes QUIT-01 + QUIT-02 by-test + dev-mode smoke; live UAT deferred to v1.2.0 ship round (CONTEXT D-09 / D-10). Promoted from backlog 2026-04-29 (former 999.1).
...
| 20. Documentation Builder feature | v1.2 | 4/4 | All plans complete 2026-05-01 (Documentation types + validator + summary.events; DocumentationBuilderDialog modal + Sections pane + AppShell drift wiring; Animation Tracks pane DnD + 11 RTL+jsdom tests; HTML export + IPC + 11 doc-export tests + 3 round-trip tests; DOC-01 + DOC-02 + DOC-03 + DOC-04 + DOC-05) — ready for `/gsd-verify-work 20` | 2026-05-01 |
