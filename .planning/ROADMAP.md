...
Plans:
- [ ] 14-01-PLAN.md — Main-side: trigger-aware suppression (D-05/D-08), sticky pending-update slot (D-03), structured logging (D-09/D-10), `update:request-pending` IPC handler, SHELL allow-list re-verification (D-12) (Wave 1, autonomous)
- [ ] 14-02-PLAN.md — Preload bridge: add `requestPendingUpdate` to `window.api` contextBridge surface (Wave 1, autonomous)
- [ ] 14-03-PLAN.md — Renderer lift: move 5 update subscriptions + `updateState` + `manualCheckPendingRef` + `<UpdateDialog>` from AppShell to App.tsx; add late-mount sticky-slot fetch (D-01..D-04) (Wave 2, autonomous)
