...
- 🚧 **v1.2 (in progress)** — 7-phase expansion (Phases 13.1, 16, 18–22) authored 2026-04-30 + Phase 22.1 inserted 2026-05-03 to close 7 HUMAN-UAT gaps from Phase 22. Closes two macOS regressions + one host-blocked carry-forward from v1.1.x (13.1 + 16 + 18); refines the UI based on tester feedback (19); adds the Documentation Builder feature filling the `.stmproj` v1 reserved `documentation: object` slot from D-148 (20); lands the two long-dormant SEEDs — atlas-less mode (21) and dims-badge override-cap (22; depends on 21); 22.1 closes 7 visual-UAT gaps from 22 (1 BLOCKER override-on-passthrough + 5 major + 1 minor). 26 REQs across UAT, UPDFIX, QUIT, UI, DOC, LOAD, DIMS categories — UPDFIX-06 closed-by-test on 2026-04-30 (regression test 14-l in `tests/renderer/app-update-subscriptions.spec.tsx`; Phase 14 lift commit 802a76e already fixes the wiring), so Phase 17 was skipped. Apple Developer ID code-signing + Sentry crash reporting declined for v1.2 (revisit at v1.3); manual-download UX (Phase 16) is the locked v1.2 answer for macOS auto-update.
...
### 🚧 v1.2 (in progress) — Phases 13.1, 16, 18–22 (7 phases; promoted from backlog + tester feedback + dormant seeds 2026-04-30)

7-phase expansion. Phase 13.1 is host-availability gated and runs opportunistically; Phase 22 depends on Phase 21; Phases 16/18 are independent macOS UX fixes; Phase 19 (UI) recommended before Phase 20 (Documentation Builder modal benefits from Phase 19's sticky-header + modal-redesign patterns). Recommended execution order: 16 → 18 → 19 → 20 → 21 → 22, with 13.1 inserted opportunistically when a Linux/macOS/Windows host becomes available. Final order is the user's call. **Phase 17 was promoted from backlog 2026-04-29 then skipped 2026-04-30** — UPDFIX-06 already closed-by-test (regression test 14-l in `tests/renderer/app-update-subscriptions.spec.tsx`; Phase 14 lift commit 802a76e moved the menu-listener subscription to App.tsx so it runs on every AppState branch including idle); D-15-LIVE-3 was observed on the v1.1.1 installed binary pre-lift, and v1.1.3+ already has the fix.
...
- [x] **Phase 16: macOS auto-update — switch to manual-download UX** — Replace Squirrel.Mac in-process swap on macOS with the existing `windows-fallback` UpdateDialog variant pattern (opens GitHub Releases page in browser instead of attempting a code-signature-validated swap that fails on ad-hoc-signed builds). Flip `SPIKE_PASSED = process.platform !== 'win32'` gate in `src/main/auto-update.ts` so macOS routes to the same path Windows already uses; rename `windows-fallback` variant → neutral name (e.g. `manual-download`); update INSTALL.md + Help copy + tests. Closes D-15-LIVE-2 (former 999.2). Promoted from backlog 2026-04-29. (completed 2026-04-30)
- [~] **Phase 17: Help → Check for Updates — fire regardless of project state** — **SKIPPED 2026-04-30** (promoted from backlog 2026-04-29). UPDFIX-06 closed-by-test: regression test 14-l in `tests/renderer/app-update-subscriptions.spec.tsx` ("Help → Check for Updates from idle calls window.api.checkForUpdates()") already locks the wiring on every AppState branch. Phase 14 lift commit 802a76e moved the menu-listener subscription from `AppShell.tsx` to `App.tsx`'s top-level `useEffect`. D-15-LIVE-3 was observed on the v1.1.1 installed binary pre-lift; v1.1.3+ already has the fix. No source change needed; verification-only phase deemed redundant.
- [x] **Phase 18: App quit broken — Cmd+Q + AppleScript do not terminate** — Renderer-side IPC subscription lift mirroring Phase 14 commit `802a76e`: move `onCheckDirtyBeforeQuit` from `AppShell.tsx` (mounted only on `loaded` / `projectLoaded`) to `App.tsx`'s top-level useEffect (always-mounted root) so the listener survives `idle` / `error` / `projectLoadFailed` AppStates. Bridge dirty signal + SaveQuitDialog dispatch back through a new `dirtyCheckRef`. No main-process or preload changes. Closes QUIT-01 + QUIT-02 by-test + dev-mode smoke; live UAT deferred to v1.2.0 ship round (CONTEXT D-09 / D-10). Promoted from backlog 2026-04-29 (former 999.1). (completed 2026-04-30)
...
- [ ] **Phase 22.1: Close Phase 22 HUMAN-UAT gaps (INSERTED)** — 7 gaps surfaced during Phase 22 visual UAT on 2026-05-03 (1 BLOCKER + 5 major + 1 minor). BLOCKER: user-applied scale overrides are silently ignored on passthrough rows — when override forces effective scale BELOW source-ratio cap the row must leave passthroughCopies[] and become a genuine resize (animator workflow for glow/blend-mode attachments below calculated optimum is currently broken). Plus DIMS-02 badge mode-awareness (atlas-source mode shows confusing "smaller than canonical" callout when displayed Source W×H IS the canonical), tooltip hover reliability (fires once then nothing — listener race or virtualization), cap-binding-aware tooltip copy (drop "Optimize will cap at source size." when peakAlreadyAtOrBelowSource), DIMS-04 generalized passthrough predicate (covers TRIANGLE peakScale=1.0× no-op rows currently Lanczos-resampled wastefully), OptimizeDialog COPY chip drops "(already optimized)" parenthetical, passthrough rows show source→target dim shape so overrides preview before Start. Source: 22-HUMAN-UAT.md ## Gaps section.
