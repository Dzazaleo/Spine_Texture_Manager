...
- [x] 08.1-02-PLAN.md — Main-side wire: handleProjectOpenFromPath threads 7 recovery fields at the SkeletonJsonNotFoundError rescue (D-159). Drives 8.1-IPC-01 GREEN. Wave 2, autonomous, depends_on [08.1-01]. ✅ 2026-04-26 — commit `dbb3ffc` (feat: 7-field rescue + parallel handleProjectReloadWithSkeleton rescue + NonRecoveryKind cast narrowing); 1 deviation (Rule 3 Blocking — scope expansion to clear all 5 Plan-01 typecheck breaks per Plan-01 Next-Readiness map + orchestrator objective); 271 passed + 4 RED + 1 todo + 1 skipped (was 270/5/1/1; +1 GREEN = 8.1-IPC-01); locked-file diffs clean. SUMMARY: `08.1-02-SUMMARY.md`.
- [x] 08.1-03-PLAN.md — Renderer-side wire: AppShell.onClickOpen reads threaded fields from typed envelope (D-160); "best-effort" comment block deleted. Drives 8.1-VR-02 GREEN. Wave 3, autonomous, depends_on [08.1-01, 08.1-02]. ✅ 2026-04-26 — commit `e081b18` (feat: replace 8 placeholder literals at AppShell.tsx:521-543 with discriminated-union field reads `originalSkeletonPath: resp.error.originalSkeletonPath` etc.; stale 7-line "best-effort" comment block deleted; replaced with 11-line D-160 explanation citing upstream Plan 08.1-02 wire site at `src/main/project-io.ts:333-343`); zero deviations (plan executed byte-for-byte); 272 passed + 3 RED + 1 todo + 1 skipped (was 271/4/1/1; +1 GREEN = 8.1-VR-02 — `expected '' to be '/a/b/proj.stmproj'` now passes since threaded `projectPath` flows through `onClickLocateSkeleton`'s `window.api.reloadProjectWithSkeleton`); all 12 plan-level acceptance grep gates PASS (7 positive `resp.error.*` field-read greps each return 1; 4 negative greps return 0); locked-file diffs clean vs Phase 5 `552389e` (cli.ts/sampler.ts/loader.ts = 0 lines, project-file.ts = 392 lines = Plan 08-02 baseline, D-171 lock holds); `npm run cli` exits 0 with CIRCLE 2.018 / SQUARE 1.500 / TRIANGLE 2.000 byte-for-byte unchanged. **Toolbar Open + Cmd+O missing-skeleton recovery now has full parity with drag-drop path.** SUMMARY: `08.1-03-SUMMARY.md`.
- [x] 08.1-04-PLAN.md — App.tsx projectLoadFailed AppState variant + recovery banner (D-161, D-162); handleLocateSkeleton + handleDismissProjectLoadFailed callbacks. Drives 8.1-VR-01 GREEN. Wave 3, autonomous, depends_on [08.1-01, 08.1-02]. ✅ 2026-04-26 — commit `342eddf` (feat: AppState union grows from 4 → 5 variants — new `projectLoadFailed` carrying `Extract<SerializableError, { kind: 'SkeletonNotFoundOnLoadError' }>` exposes 7 threaded recovery fields directly inside `state.error.*` reads via discriminated-union narrowing utility — T-08.1-06 wrong-arm-field-reads mitigated at compile time; handleProjectLoad gains nested if-branch routing `error.kind === 'SkeletonNotFoundOnLoadError'` to new variant; new handleLocateSkeleton useCallback mirrors AppShell.onClickLocateSkeleton structurally — `window.api.locateSkeleton` → `window.api.reloadProjectWithSkeleton` chain; new handleDismissProjectLoadFailed transitions to `idle`; new render branch with `role="alert"` recovery banner card mirroring AppShell.tsx:746-770 visually — `border + rounded-md + bg-panel + bg-danger 1px accent + Skeleton not found:` prefix + Locate skeleton… / Dismiss buttons; Tailwind v4 literal-class discipline preserved); 1 deviation (Rule 3 Blocking — test fixture wait-mechanism for 8.1-VR-01: spec from Plan 08.1-01 used `await Promise.resolve()` x3 + `screen.queryByRole` which never flushes React 19's concurrent state-update scheduler — verified empirically with 10-microtask scratch test where banner never appeared after 10 microtasks but appeared after a 50ms macrotask delay; replaced with `await screen.findByRole('alert')` — canonical testing-library async-DOM wait pattern; no assertion semantics changed; only the wait mechanism; fix scoped to single 8.1-VR-01 spec); 273 passed + 2 RED + 1 todo + 1 skipped (was 272/3/1/1; +1 GREEN = 8.1-VR-01 — `expected null to be truthy` → `Test Files 1 passed (1) | Tests 1 passed | 8 skipped (9)`); **CONTEXT D-166 cumulative target 270→≥273 passed achieved**; all 10 plan-level acceptance grep gates PASS; AppShell.tsx untouched (recovery surface is App.tsx-local per WR-09 fix-a / minimal-blast-radius rationale); locked-file diffs clean vs Phase 5 `552389e` (cli.ts/sampler.ts/loader.ts = 0 lines); `npm run cli` exits 0 with CIRCLE 2.018 / SQUARE 1.500 / TRIANGLE 2.000 byte-for-byte unchanged (sampling 23.8 ms at 120 Hz); Layer 3 invariant intact (no `src/core/*` imports in App.tsx). **Drag-drop missing-skeleton recovery now reachable end-to-end** — was unreachable pre-Phase-8.1 because routing landed in `error` and AppShell never mounted. SUMMARY: `08.1-04-SUMMARY.md`.
- [x] 08.1-05-PLAN.md — VR-03 fix: useRef callback bridge (App.tsx) + onBeforeDropRef prop + useEffect registration (AppShell.tsx) + cancelAction extension to saveQuitDialogState + SaveQuitDialog.onCancel invocation (D-163, D-164). DropZone.tsx and SaveQuitDialog.tsx untouched (D-165). Drives 8.1-VR-03a + 8.1-VR-03b GREEN. Wave 4, autonomous, depends_on [08.1-01, 08.1-04]. ✅ 2026-04-26 — commits `653e09a` (feat: AppShell extends with optional `onBeforeDropRef?: MutableRefObject<onBeforeDropFn|null>` prop + `cancelAction?` extension to saveQuitDialogState (D-164) + new useEffect([isDirty, onBeforeDropRef]) registers Promise factory resolving true on Save/Don't Save via pendingAction OR false on Cancel via cancelAction; cleanup nulls the ref on unmount; SaveQuitDialog onCancel handler invokes `saveQuitDialogState.cancelAction?.()` before clearing state — optional invocation pattern means existing 'quit' callers don't change behavior since Electron handles the abort) + `93440c2` (feat: App.tsx adds `useRef` import + new `beforeDropRef = useRef<onBeforeDropFn|null>(null)` + stable `handleBeforeDrop = useCallback(empty deps)` wrapper that dereferences `beforeDropRef.current?.(...) ?? true` at call time + DropZone wired with `onBeforeDrop={handleBeforeDrop}` + both AppShell mounts (loaded + projectLoaded branches) gain `onBeforeDropRef={beforeDropRef}`). 2 deviations auto-fixed (orchestrator explicitly heads-up'd both as expected scope): (1) Rule 3 - Blocking React 19 wait-mechanism — replaced `await Promise.resolve()` x3 + `screen.queryByText` with `await screen.findByText/findByRole` (testing-library canonical async-DOM wait under React 19 concurrent rendering — same Rule 3 fix applied in Plan 08.1-04 for 8.1-VR-01); (2) Rule 1 - Bug in test fixture target — pre-existing specs from Plan 08.1-01 (committed in `d756dd9`) targeted TRIANGLE for the dirty-mutation row but `makeSummary()` peaks contain only CIRCLE + SQUARE → silent `.catch([])` skipped the override-mutation flow → isDirty never flipped → SaveQuitDialog never mounted → assertion failed with misleading `expected undefined to be truthy`; retargeted to CIRCLE row + replaced silent `.catch` with hard-failing `await screen.findByText(/^CIRCLE$/i)` + targeted Scale `<td>` (where onDoubleClick lives) via `within(circleRow).getByText(/0\.500×/)`. 275 passed + 0 RED + 1 todo + 1 skipped (was 273/2/1/1; +2 GREEN = 8.1-VR-03a + 8.1-VR-03b — `expected undefined to be truthy` → `Test Files 1 passed (1) | Tests 9 passed (9)`); **Surpasses CONTEXT D-166 final target 270→≥273 passed at 275.** All Task 1 + Task 2 plan-level acceptance grep gates PASS; pre-existing deferred `scripts/probe-per-anim.ts:14:31` TS2339 unchanged; **DropZone.tsx and SaveQuitDialog.tsx untouched** (D-165 lock holds — bodyCopyFor already supports both reasons). Locked-file invariants vs Phase 8.1 baseline `90dd202` all clean (0 lines): `scripts/cli.ts`, `src/core/sampler.ts`, `src/core/loader.ts`, `src/core/project-file.ts` (D-171). `npm run cli` exits 0 with CIRCLE 2.018 / SQUARE 1.500 / TRIANGLE 2.000 byte-for-byte unchanged (sampling 23.8 ms at 120 Hz). Layer 3 invariant intact (no `src/core/*` imports in App.tsx or AppShell.tsx). **VR-03 closed end-to-end** — dropping a new .json/.stmproj on a dirty session mounts SaveQuitDialog with the correct reason discriminator; Cancel aborts the drop (overrides survive); Don't Save proceeds (overrides discarded with the AppShell unmount per D-167). **Phase 8.1 implementation scope COMPLETE** — all 5 specs (8.1-VR-01, 8.1-VR-02, 8.1-VR-03a, 8.1-VR-03b, 8.1-IPC-01) GREEN; only Plan 08.1-06 close-out remains. SUMMARY: `08.1-05-SUMMARY.md`.
- [x] 08.1-06-PLAN.md — Close-out: automated exit-criteria sweep + 08.1-VALIDATION.md per-plan map + 3-reproducer manual UAT + ROADMAP.md plan-checkbox flips + STATE.md advance. Wave 5, has checkpoint, depends_on [08.1-01..05]. ✅ 2026-04-26 — commit `d8e63ae` (close-out: 275/277 tests + locked-file invariants intact + CLI byte-for-byte unchanged + 3-reproducer manual UAT signed off — VR-01 ✓ / VR-02 ✓ / VR-03 A+B ✓; 1 adjacent discovery flagged for Phase 08.2 — Cmd+O blocked when app is in error state).

---

### Phase 08.2: File-menu surface + Cmd+O accelerator gating fix (INSERTED)

**Goal:** Fix the Cmd+O-blocked-during-error-state issue discovered in Phase 08.1 UAT, AND add the missing native File menu items (Open, Open Recent, Save, Save As) so the app behaves like a standard macOS/Windows desktop document app. Both pieces touch the same Electron menu + IPC accelerator surface, so they ship together.

**Requirements**: TBD (open File menu — UX standard; Open Recent persistence; Save / Save As menu parity with Cmd+S behavior; Cmd+O accelerator must work regardless of error/banner state).
**Depends on:** Phase 08.1 (recovery banner surface + reloadProjectWithSkeleton IPC are prerequisites for the Cmd+O fix).
**Plans:** 5/5 plans complete

Plans:
- [x] 08.2-01-recent-storage-PLAN.md — Wave 1 storage layer: src/main/recent.ts (loadRecent / addRecent / clearRecent / validateRecentFile + RecentFileV1 type) with hand-rolled type guard mirroring validateProjectFile (D-156) + atomic-write idiom mirroring image-worker (Phase 6); tests/main/recent.spec.ts 9 cases (a-i) covering D-177/D-178/D-180 + hygiene grep ("only `app` from electron"). Wave 1, autonomous, depends_on []. Underwrites: F9.1 / F9.2 surface for Open Recent persistence.
- [x] 08.2-02-app-menu-builder-PLAN.md — Wave 2 native menu: buildAppMenu + applyMenu inline in src/main/index.ts (PATTERNS Option A) with full standard template per D-174 (App / File / Edit / View / Window / Help) + role:* (D-188) + accelerators (D-185); module-scope mainWindowRef + currentMenuState + getter exports for Plan 03; initial menu install at app.whenReady(); autoHideMenuBar:true preserved (D-186); tests/main/menu.spec.ts 7 cases (a-g) covering enabled-state matrix. Wave 2, autonomous, depends_on [08.2-01].
- [x] 08.2-03-ipc-preload-wiring-PLAN.md — Wave 3 IPC + preload glue: ipcMain.on(menu:notify-state) handler with trust-boundary validation (D-181) in src/main/ipc.ts; addRecent + applyMenu threaded into handleProjectSaveAs and handleProjectOpenFromPath success arms only (D-180); src/preload/index.ts gains 5 new contextBridge methods (notifyMenuState + 4 onMenu* with Pitfall 9 listener-identity); src/shared/types.ts Api interface extended; tests/main/ipc.spec.ts (NEW) 2 cases. Wave 3, autonomous, depends_on [08.2-01, 08.2-02].
- [x] 08.2-04-renderer-wiring-PLAN.md — Wave 4 renderer: App.tsx subscribes to all 4 menu-event channels with Pitfall 9 + 15 cleanup; per-AppState notifyMenuState push for idle/error/projectLoadFailed (D-187 — the canonical Cmd+O fix); appShellMenuRef parallel to beforeDropRef (D-175); AppShell.tsx DELETES keydown listener (D-176); ADDS notifyMenuState push + appShellMenuRef registration useEffects; AppShellProps gains optional appShellMenuRef; dedicated onClickSaveAs callback extracted (PATTERNS heads-up). Wave 4, autonomous, depends_on [08.2-03].
- [x] 08.2-05-renderer-tests-and-close-out-PLAN.md — Wave 5 verification: tests/renderer/save-load.spec.tsx adds 8.2-MENU-01..06 (incl. canonical 8.2-MENU-04 Cmd+O-on-recovery-banner regression test); retires the Keyboard shortcuts describe block at lines 92-121 (D-176); automated exit-criteria sweep + 08.2-VALIDATION.md per-plan map; checkpoint:human-verify on 4 reproducers; STATE.md + ROADMAP.md advance. Wave 5, has checkpoint, depends_on [08.2-02, 08.2-04].

---

## Phase 9: Complex-rig hardening + polish

**Goal:** Sampler offloaded to a real `worker_threads` Worker (D-190 unconditional; D-193 path-based protocol; D-194 progress + cancel via terminate); UI virtualization in BOTH panels via TanStack Virtual at threshold N=100 (D-191/D-192/D-195/D-196 — outer card list in regular DOM, per-card inner row list virtualizes); SettingsDialog under Edit→Preferences (Cmd/Ctrl+,) exposing samplingHz with re-sample on change; rig-info tooltip on the filename chip showing `skeleton.fps: N (editor metadata — does not affect sampling)` aligned to sampler.ts:41-44; HelpDialog under Help→Documentation with 7 canonical static React sections + allow-listed external links.

**Depends on:** Phase 8.2 closed (eb97923 baseline).
...
Plans:
- [x] 09-01-wave0-scaffold-PLAN.md — Wave 0 scaffolds: install `@tanstack/react-virtual@^3.13.24` (D-192); RED test scaffolds for the 7 new spec files; extensions to tests/main/ipc.spec.ts (sampler:cancel block) + tests/arch.spec.ts (Phase 9 named anchor for src/main/sampler-worker.ts). Wave 0, autonomous, depends_on []. Underwrites: every Plan 02-07 task can claim a real automated verify command from day one.
- [x] 09-02-sampler-worker-PLAN.md — Wave 1 N2.2 GATE: src/main/sampler-worker.ts (greenfield Node `worker_threads` Worker; runSamplerJob exported for in-thread vitest unit tests + parentPort entrypoint for Worker spawn) + src/main/sampler-worker-bridge.ts (runSamplerInWorker Promise-wrapper + getSamplerWorkerHandle for cancel) + src/main/ipc.ts `'sampler:cancel'` handler + src/main/project-io.ts call-site refactor (lines 437-441 + 640-643) + src/preload/index.ts onSamplerProgress + cancelSampler + AppShell.tsx indeterminate sampling spinner + cancel-on-mid-sample-Open hook + electron.vite.config.ts sampler-worker bundle. fixtures/Girl wall-time gate <8000 ms. Wave 1, autonomous, depends_on [09-01].
- [x] 09-03-globalmax-virtualization-PLAN.md — Wave 2 GlobalMaxRenderPanel.tsx threshold-gated TanStack Virtual at N=100; sticky thead via CSS (Pitfall 1 — never on tr); overflow-anchor:none; getItemKey memoized to attachmentKey; existing sort/search/checkbox/OverrideDialog/focusAttachmentName-jump preserved in BOTH paths. Wave 2, autonomous, depends_on [09-01]. Parallel-safe with 09-04.
- [x] 09-04-anim-breakdown-virtualization-PLAN.md — Wave 2 AnimationBreakdownPanel.tsx per-card inner-row TanStack Virtual with measureElement variable-height (D-196 — outer card list stays in regular DOM); collapse/expand cycle preserves filter query; OverrideDialog mounts from virtualized rows. Wave 2, autonomous, depends_on [09-01]. Parallel-safe with 09-03.
- [x] 09-05-menu-surface-PLAN.md — Wave 3 menu surface scaffolds: Edit menu replaces role:'editMenu' with custom submenu containing standard Edit roles + Preferences… (CommandOrControl+,) firing 'menu:settings-clicked'; Help submenu (role:'help' placeholder fill) gains Documentation firing 'menu:help-clicked'; src/main/ipc.ts gains `'shell:open-external'` handler with hardcoded SHELL_OPEN_EXTERNAL_ALLOWED Set (T-09-05-OPEN-EXTERNAL); src/preload/index.ts exposes onMenuSettings + onMenuHelp + openExternalUrl; tests/main/menu.spec.ts + ipc.spec.ts extended. Wave 3, autonomous, depends_on [09-01].
- [x] 09-06-settings-and-tooltip-PLAN.md — Wave 4 SettingsDialog.tsx (NEW; modal with samplingHz dropdown 60/120/240/Custom + validation) + AppShell.tsx onMenuSettings subscription + samplingHz re-sample useEffect (RESEARCH §Pitfall 7) + rig-info tooltip on filename chip with editor-metadata wording aligned to sampler.ts:41-44 + src/main/summary.ts editorFps surfacing + src/main/ipc.ts `'project:resample'` channel + handleProjectResample worker-dispatched re-sample. Wave 4, autonomous, depends_on [09-02, 09-05].
- [x] 09-07-help-dialog-PLAN.md — Wave 5 HelpDialog.tsx (NEW; static React with 7 canonical sections per Claude's Discretion; NO markdown library — RESEARCH §Recommendations #4) + AppShell.tsx onMenuHelp subscription; external link buttons call window.api.openExternalUrl with URLs allow-listed in 09-05 SHELL_OPEN_EXTERNAL_ALLOWED. Section 7 wording aligned to sampler.ts:41-44 + 09-06 tooltip. Wave 5, autonomous, depends_on [09-05, 09-06].
- [x] 09-08-close-out-PLAN.md — Wave 6 close-out: automated exit-criteria sweep (vitest + typecheck + electron-vite build + 6 locked-file diff vs eb97923 + CLI smoke); 09-VALIDATION.md per-plan map filled and all 18 behaviors flipped to ✅ green; manual UAT checkpoint (5 reproducers PASS, signed off 2026-04-26); STATE.md + ROADMAP.md advance; final commit. Wave 6, has checkpoint:human-verify, depends_on [09-02, 09-03, 09-04, 09-05, 09-06, 09-07].

**Exit criteria:**
- Complex rig (fixtures/Girl per D-189) samples in <10 s — wall-time test enforces <8000 ms with 2000 ms margin.
- No dropped UI frames during sampling on the complex rig (manual DevTools Performance verification per 09-VALIDATION.md §Manual-Only Verifications).
