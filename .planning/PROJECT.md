...
## Current Milestone: v1.5 Override Routing + Coverage Hardening

**Goal:** Fix the atlas-source / atlas-less override bleed bug, close two unverified Spine 4.2 timeline coverage gaps with fixtures, and burn down three long-lived tech-debt / UAT carry-forwards.

**Progress:** Phase 36 (Split Overrides Per Loader Mode, OVR-01..07) — COMPLETE 2026-05-13. 5 plans, 14/14 must-haves verified, 1076 tests passing (+3 CR-01 main-side regression tests). SEED-007 closed. Phase 38 (Phase 4 Code-Review Polish Pass, POLISH-01..03) — COMPLETE 2026-05-13. 3 plans, 4/4 must-haves verified. POLISH-AUDIT enumerates 7 deferred Phase 4 findings (IN-01..IN-06 + WR-03): 1 applies (IN-02 drag-to-cancel overlay guard — fixed in `OverrideDialog.tsx` via `onMouseDown` + `e.target === e.currentTarget`), 5 no-ops swept by Phase 6 Gap-Fix R6 + Phase 27 QA-01..04, 1 skip (IN-04 `highlightMatch` panel duplication — intentional per Phase 2/3 self-contained-panel pattern). v1.0-era pending todo retired.

**Target features:**
- ✓ Split overrides per loaderMode (SEED-007 — closed Phase 36 — schema-additive `overridesAtlasLess` field; legacy v1.3.x routing by saved `loaderMode`; mode-switch leaves inactive bucket untouched; decisions 1, 2-A, 3-A LOCKED; CR-01 resample-handler bucket-routing bug surfaced + fixed in code-review-fix cycle)
- Timeline coverage hardening (SEED-005 Level B — source audit of RGBA2Timeline + InheritTimeline in spine-core; new fixture exercising InheritTimeline detach on a rotating bone; fixture or synthetic test for RGBA2)
- Long-lived todo burndown (✓ Phase 4 code-review follow-up — closed Phase 38; Phase 20 Doc Builder Windows DnD UAT; Phase 31 Windows admin DnD release UAT — last two host-blocked, ship if Win host available)

**Out of scope (deferred):**
- SEED-006 (full Spine 4.3 runtime port) — gated on `@esotericsoftware/spine-core@4.3.x` npm publish; no action this milestone.
