...
## Current State (post v1.5.1; v1.6 in progress — Phase 42 complete)

**v1.6 in progress:** Spine 4.3 Runtime Port (Dual-Runtime) — see `## Current Milestone` below. SEED-006 trigger fired 2026-05-16 (`npm view @esotericsoftware/spine-core@latest` → `4.3.0`; `@esotericsoftware/spine-player@latest` → `4.3.0`). **Phase 42 COMPLETE 2026-05-16** (pre-v1.6 4.2 baseline + npm alias + boundary scaffolding; 5/5 plans, re-verified passed 5/5 — SAFE-01/RT-01/RT-03/RT-04/CI-01). The dual-install is live: `@esotericsoftware/spine-core@4.3.0` canonical + `spine-core-42`=`4.2.111` exact-pinned alias; frozen ancestry chain A→B→repoint→C→D is the SAFE-01 acceptance test. Renderer still imports the 4.2-only `MixBlend` (11 `tests/renderer/*` suites red) — an intended, ROADMAP-Phase-47-owned state, not a regression. Next: Phase 43 (runtime-adapter facade + verified 4.3 API mapping).
