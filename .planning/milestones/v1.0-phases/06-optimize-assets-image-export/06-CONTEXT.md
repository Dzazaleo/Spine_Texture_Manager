...
- **D-110: Output pixel dimensions = `Math.round(sourceW × effectiveScale)` × `Math.round(sourceH × effectiveScale)`.** Both axes use the SAME `effectiveScale` (locked from memory: uniform-only export sizing — anisotropic export breaks Spine's UV sampling contract; the engine samples meshes via UVs against whatever dims the exported image actually has, and squishing one axis deforms the rendered result). Math.round = round-half-away-from-zero (JavaScript default). Minimizes max pixel error vs floor (which silently drops a pixel at peakScale 0.999) or ceil (which over-allocates by <1px). Sub-pixel error has zero visible effect on Lanczos3 output. Test fixture: `Math.round(127.5)` = 128; `Math.round(127.4)` = 127.
  ...
  **D-110 Round 5 amendment (2026-04-25 human-verify Step 1 re-test):** The Math.round formulation above is SUPERSEDED by ceil + ceil-thousandth semantics after a 1-pixel under-allocation surfaced on JOKER/FACE (peakScale 0.36071, source 811×962 → Math.round(962 × 0.36071) = 347 but per-axis peak demanded ≥ 347.99). The locked refinement, applied byte-identically to `src/core/export.ts` + `src/renderer/src/lib/export-view.ts` + the renderer-side `computeExportDims` helper:
    1. **effectiveScale is rounded UP to the nearest thousandth** via `Math.ceil(s * 1000) / 1000` AFTER override/peakScale resolution AND BEFORE the ≤ 1.0 clamp. The displayed `0.361×` (panel "Scale" column, via `src/core/analyzer.ts` scaleLabel which mirrors the same rounding) becomes a guaranteed lower bound — a user reading the panel and applying 36.1% in Photoshop never produces a smaller image than the app exports.
    2. **Output dims use `Math.ceil(sourceDim × effectiveScale)`** instead of `Math.round(...)`. Math.ceil guarantees output dim ≥ per-axis peak demand on both axes; Math.round could drop a pixel when fractional product < .5.
    3. **Aspect ratio preserved within sub-pixel tolerance.** Same effectiveScale on both axes (D-110 uniform invariant unchanged); ceil applied per-axis. Worst-case anisotropy from per-axis ceil is ≤ 1 px on a single axis — invisible after Lanczos3.
    4. **Panel "Peak W×H" column now shows export dims, NOT world-AABB.** Previously the column showed `worldW×worldH` (a different measurement that can violate aspect ratio under rotation or mesh deformation), causing the user to see a different number from what the optimize dialog reported. Round 5 unifies the column with the optimize dialog via the renderer-side `computeExportDims` single-source-of-truth helper. World-AABB moves to a hover tooltip on the cell ("World AABB at peak: WxH") — diagnostic info for power users.
