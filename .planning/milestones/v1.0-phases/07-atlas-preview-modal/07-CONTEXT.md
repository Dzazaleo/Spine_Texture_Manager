...
### Phase 7 gap-fix amendment (2026-04-25 UAT signoff)

- **D-139: Atlas canvas display-size auto-fits the modal content area; backing-store stays at atlas page resolution.** Per Phase 7 UAT Gap 4 finding (07-HUMAN-UAT.md): canvas overflowed the modal at smaller viewports because explicit `style.width=2048px; style.height=2048px` did not preserve the modal's `max-h-[90vh] max-w-[95vw]` envelope. Amendment: keep `canvas.width = page.width × dpr` and `canvas.height = page.height × dpr` (backing store unchanged for `drawImage` fidelity), but drop explicit pixel `style.width` / `style.height`; instead wrap canvas in an `aspect-[1/1]` flex item with `width: 100%; height: 100%` and let CSS contain the canvas to the available square area inside the modal main pane. Parent main pane swaps `overflow-auto` → `overflow-hidden` to enforce the no-scrollbar constraint. Atlas Preview is informational only — its display size should track viewport, not backing-store. Closes Gap 4. Rejected: keep explicit pixel sizing + add scrollbar (user-rejected during UAT — "scrollbar is wrong"); shrink the backing-store to viewport (loses drawImage fidelity at retina dpr; defeats Phase 7 D-133's source-pixel intent).
