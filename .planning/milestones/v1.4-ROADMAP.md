...
- ✅ **v1.3.1 Correctness & Refinements** — Phases 29–31 (shipped 2026-05-09; v1.3.1 final; 3 phases / 16 plans / 20 REQs — REGION-01..07, PREVIEW-01, BUFFER-01..03, LOAD-05..07, PANEL-08..11, PLATFORM-01, TOOLTIP-01; closes the per-region dedup correctness bug surfaced post-v1.3 ship via analyzer.ts attachmentName→regionName key flip, adds user-configurable safety-buffer % in Optimize, lands targeted UX refinements) — full archive at [.planning/milestones/v1.3.1-ROADMAP.md](milestones/v1.3.1-ROADMAP.md)
- 🚧 **v1.4 Spine 4.3 Forward-Compat + Rotated Atlases** — Phases 32–34 (in progress; 6 REQs — COMPAT-01, COMPAT-02, ATLAS-01, ATLAS-02, ATLAS-03, ATLAS-04; replaces today's cryptic 4.3-beta `IK Constraint not found` error with an actionable re-export-as-4.2 advisory + bold `text-danger` v4.2 disclosure on the drop zone; removes the `RotatedRegionUnsupportedError` hard-throw with full rotated-atlas-region support including AABB W↔H swap and ExportPlan dim swap; plants SEED-006 for the full 4.3 runtime port queued for post-`spine-core@4.3` npm publish)
...
- [x] **Phase 26.1: UI polish — visual wins** — `#232732` surface tokens, full-width panels, zebra rows, toolbar height unification, missing-row full bg fill, inline SVG warning triangle (16×16), atlas-less images counter, danger-themed problem-zone panel headers, stronger section headers. (UI-06, UI-07, UI-10) (completed 2026-05-04)
- [x] **Phase 26.2: UI polish — tab restructure + icon audit** — 2-tab strip lifted to dedicated sub-toolbar row (resolves AP-01 anti-pattern); filmstrip + bar-chart icons added; "orphaned" → "unused" rename; UnusedAssetsPanel inline SearchBar removed; 4 fill warning-triangle icons converted to stroke via shared `WarningTriangleIcon`. 3-tab restructure DROPPED per user redesign — alert-bar layout retained. (UI-08) (completed 2026-05-04)
- [x] **Phase 27: Code quality sweep** — Functional `setSelected` updater (closes stale-closure race); OverrideDialog empty-input Apply guard; localeCompare numeric sort; dead `open` prop removal. (QA-01..04) (completed 2026-05-05)
...
### ✅ v1.3.1 Correctness & Refinements (Phases 29–31) — SHIPPED 2026-05-09
...
### 🚧 v1.4 Spine 4.3 Forward-Compat + Rotated Atlases (Phases 32–35) — IN PROGRESS
...
- [x] **Phase 35: Region-keyed export plan — propagate Phase 29 dedup to Optimize modal + Atlas Preview optimized mode** — Complete the Phase 29 region-keyed dedup contract on the two surfaces that still consume attachment-name-collapsed data. `buildExportPlan` in `src/core/export.ts` (and its renderer parity copy `src/renderer/src/lib/export-view.ts` per the lockstep duplication invariant) iterates `summary.peaks`, which is deduped by `attachmentName` via `analyzer.ts:dedupByAttachmentName`. On multi-skin atlas-source projects where skin-namespaced regions share a base attachment name (e.g. four skins each declaring `CARDS_L_HAND_1`), 160 distinct atlas regions collapse to ~23 export rows — feeding both the **Optimize Assets** modal header/list and the **Atlas Preview** optimized-mode tile expansion, both of which call into `buildExportPlan`. Migrate the iteration source to `summary.regions` (RegionRow[]) so all four export-plan consumers honor the region-keyed contract Phase 29 established for the Global table. Atlas-less path is unaffected (`analyzeRegions` is mode-agnostic). Includes regression-fixture coverage via the existing `fixtures/SKINS/JOKERMAN_SPINE.json` (7 skins, 160 regions). (REQs: DEDUP-04, DEDUP-05, DEDUP-06) [Root cause: `.planning/debug/skins-optimize-undercount.md`] (completed 2026-05-12)
...
**Goal**: After this phase, four independent UX papercuts are closed — source-toggle controls clearly indicate when they're unavailable (no atlas / no images folder); Animation Breakdown panel cards default to collapsed with bulk-expand/collapse toolbar buttons; Windows admin sessions get a clear drag-drop unavailability message instead of silently broken DnD targets; and the ExtrapolationIcon up-arrow on Peak W×H cells reliably surfaces its hover tooltip again (regression).

**Depends on**: Phase 26.2 (toolbar h-8 button style + 2-tab subtoolbar layout — bulk Expand all/Collapse all buttons inherit the same style). Phase 22.1 G-02 (DimsBadge React-managed tooltip primitive — reuse candidate for TOOLTIP-01 fix-shape (c)). No code-level dependencies on Phases 29 / 30.

**Requirements**: LOAD-05, LOAD-06, LOAD-07, PANEL-08, PANEL-09, PANEL-10, PANEL-11, PLATFORM-01, TOOLTIP-01

**Background**: Four separate user-friction surfaces, each small enough to batch into one phase per granularity calibration. None require new design decisions; TOOLTIP-01 was folded mid-discussion (2026-05-08) as a fourth qualifying small-fix:
- **LOAD-05..07** — Today the source-toggle ("Use Atlas as Source" / "Use Images Folder as Source") shows enabled-state styling even when the toggled source doesn't exist; clicking surfaces an error. Greying-out + tooltip is the established pattern (mirrors disabled toolbar button state from Phase 26.1). Filesystem state probed in `src/main/summary.ts` via `fs.existsSync` and threaded through `SkeletonSummary` (new fields `hasAtlasFile`, `hasImagesDir`).
...
- **PLATFORM-01** — On Windows, when the app runs as administrator, the OS UIPI message-filter blocks DnD events from non-elevated processes (Explorer). Today the user drops a file and nothing happens — no error, no feedback. Fix: detect elevation at startup via one-shot `net session` exec in `src/main/`, disable drop targets, surface a clear message routing the user to File → Open or to relaunch unprivileged. UIPI message-filter workaround is explicitly OUT of scope (Microsoft-discouraged for security; see Out-of-Scope section). Windows-only — Linux is host-blocked, macOS doesn't have an equivalent elevation surface.
- **TOOLTIP-01** — On Peak W×H cells with `peakScale > 1`, the up-arrow `ExtrapolationIcon` no longer surfaces its SVG `<title>` tooltip on hover; the parent `<td title="...">` wins. Same regression on both `GlobalMaxRenderPanel.tsx` (line 558-577) and `AnimationBreakdownPanel.tsx` (line 798-803). The icon's doc-comment claims SVG `<title>` reliably wins — second known regression of this surface, so a diagnose-first spike runs before fix design (per memory `feedback_narrow_before_fixing.md`). Fix shape branches by diagnosis result; candidates documented in `31-CONTEXT.md` D-D-02.

**Constraints to preserve**:
- **Layer 3 invariant** — no DOM/electron/sharp imports in `src/core/`. All four changes are renderer-only or main-process-only (elevation detection + filesystem probes live in `src/main/`).
...
- **Sibling-symmetry** — TOOLTIP-01 fix MUST land in both call sites or in the shared `ExtrapolationIcon` component (Phase 22.1 D-04 / Phase 26.2 D-06 single-source-component discipline).
- **min-h-screen anchor** (memory `project_layout_fragility_root_min_h_screen.md`) — PLATFORM-01 inline DropZone replacement must not regress the AppShell root `min-h-screen` invariant.
...
**Goal**: After this phase, dropping a Spine 4.3-beta-exported `.json` produces a clear, actionable error message routing the user to the supported re-export-as-Version-4.2 editor workflow — replacing today's misleading `IK Constraint not found: <name>` (or `Transform constraint not found: <name>`) error from spine-core 4.2's animation reader. The initial drop-zone advisory makes the v4.2-only support visible *before* the user drops a file, with bold `text-danger` emphasis on the `v4.2` token. SEED-006 is planted at phase close carrying the costed inventory for the full 4.3 runtime port (queued for post-`spine-core@4.3` npm publish).

**Depends on**: Nothing internal to v1.4 (first phase of the milestone). Reuses `SpineVersionUnsupportedError` already exported from `src/core/errors.ts` (existing class — no new error type required). Touches the same drop-zone surface as the v1.0 Phase 1 JSON drop-load (App.tsx:622) and the v1.0 Phase 0 loader (`src/core/loader.ts`). Layer 3 invariant preserved — the 4.3 sniff is pure JSON inspection, no DOM/Electron/sharp imports.

**Requirements**: COMPAT-01, COMPAT-02

**Background**: User's team already has 4.3-beta exports failing today (`fixtures/test_4.3/jokerman/JOKERMAN_SPINE.json` and `fixtures/test_4.3/girl/TOPSCREEN_ANIMATION_JOKER.json` — proprietary, gitignored alongside the existing Jokerman/Girl rigs). The full mechanism is documented in SEED-003: 4.3 unifies all constraint definitions under `root.constraints` (with a `type` discriminator) instead of the legacy four arrays (`root.ik`, `root.transform`, `root.path`, `root.physics`). Animation timeline keys did NOT rename, so spine-core 4.2's reader walks `animations.*.ik[name]` happily, but the underlying `IkConstraintData[]` is empty — `findIkConstraint("L_LEG_1_IK")` returns `null` and the animation reader throws. The `SpineVersionUnsupportedError` class already exists in `src/core/errors.ts` (originally added for related version-gating in earlier milestones); we reuse it verbatim with the v1.4 message. SEED-006 captures the post-stable port: 5 sampler renames (`setToSetupPose` → `setupPose`, `setSlotsToSetupPose` → `setupPoseSlots`, `state.setAnimationWith` → `state.setAnimation`, `slot.getAttachment()` → `slot.pose.attachment`, plus the `animState.update`/`apply` lifecycle adjustments), 2 bounds signature changes (`RegionAttachment.computeWorldVertices` adds `vertexOffsets`; `VertexAttachment.computeWorldVertices` adds `skeleton` first arg), `slot.pose` access pattern, slider-constraint timeline validation against a dedicated fixture, and vendoring strategy (git submodule + tsc, npm fork, or wait-for-publish).

**Constraints to preserve**:
- **Layer 3 invariant** — the 4.3 sniff lives in `src/core/loader.ts` BEFORE `SkeletonJson.readSkeletonData` is called; pure JSON inspection, no DOM/electron/sharp imports.
- **Reuse existing error type** — `SpineVersionUnsupportedError` is already exported from `src/core/errors.ts`; don't introduce a new error class. The discriminated-union typed-error envelope from v1.0 Phase 8 (D-158/D-171) catches it at the IPC boundary.
- **No 4.3 schema parsing or shimming** — Out-of-Scope per REQUIREMENTS.md (SEED-003 Option B is labelled HIGH trap risk; mid-beta `uniform`→`scaleY` rename at `4.3.73-beta` is precedent that the schema is not frozen). The detect-and-warn path is deliberately non-translating.
- **Drop-zone copy uses existing tokens** — `text-danger` + `font-bold` per Tailwind v4 `@theme inline` token discipline; no new token introduction. Mirrors the v1.3 Phase 26.1 problem-zone styling.
- **Drop-zone copy locus is App.tsx:622** — confirmed anchor in the orchestrator inventory; the line wraps the existing `<p>` advisory ("Drop a `.spine` JSON file anywhere in this window"). The v4.2 token is prepended/integrated, not duplicated as a separate sibling element.
- **`SIMPLE_TEST.json` continues to load unchanged** — `"spine": "4.2.43"` baseline never trips the 4.3 sniff. Regression-locked via existing `tests/core/loader.spec.ts`-shaped coverage.
- **No Spine 4.3 runtime work** — full port is OUT OF SCOPE; queued via SEED-006 plant. PORT-01..04 future requirements remain in REQUIREMENTS.md → Future Requirements.

**Success Criteria** (what must be TRUE):
  1. User opens the app with no project loaded; the drop-zone advisory at App.tsx:622 calls out the supported Spine version explicitly, with the `v4.2` token rendered in bold `text-danger` styling. The supported-version disclosure is visible *before* the user drops any file. (COMPAT-02)
  2. User drops a Spine 4.3-beta-exported `.json` (e.g. `fixtures/test_4.3/jokerman/JOKERMAN_SPINE.json` with `"spine": "4.3.88-beta"`); the loader detects the 4.3 shape (presence of `root.constraints` array OR `skeleton.spine` semver `≥ 4.3`) and surfaces the actionable error message *"This app currently supports Spine v4.2. Re-export from your 4.3 editor as Version 4.2 (supported downgrade) and try again."* in the UI. The misleading `IK Constraint not found: <name>` (or `Transform constraint not found: <name>`) error is no longer reachable for 4.3-beta JSONs. (COMPAT-01)
  3. User drops the in-repo `fixtures/SIMPLE_PROJECT/SIMPLE_TEST.json` (`"spine": "4.2.43"`); the file loads successfully into the Global + Animation Breakdown panels with no version warning surfaced — the 4.3 sniff is correctly inert for 4.2 JSONs. (COMPAT-01 negative case)
...
**Goal**: After this phase, atlas-source projects packed with Spine's `rotation: true` toggle load and process correctly end-to-end. The hard-throw `RotatedRegionUnsupportedError` is removed; rotated regions propagate through the analyzer like any other region; AABB calculations swap source W↔H so world-space bounds match what the runtime would render at identity scale; ExportPlan output dimensions reflect the visually-correct (unrotated) W×H so animators get exported per-region PNGs whose dimensions match the unrotated source. A regression fixture exercises the rotated-region code path under `fixtures/` and is locked by core unit tests covering ATLAS-01..03.

**Depends on**: Phase 32 (no functional dependency, but the v1.4 milestone ships sequentially — COMPAT first, ATLAS second). Reuses the v1.0 atlas loader (`src/core/loader.ts`) + bounds calculation (`src/core/bounds.ts`) + ExportPlan builder (likely `src/main/export-plan.ts` or equivalent — exact locus confirmed during planning). Atlas-less mode (`src/core/synthetic-atlas.ts`) is unaffected — synthetic atlas never packs with rotation, so the rotation branch is statically unreachable from the atlas-less code path. Memory `project_strict_loadermode_separation.md` discipline preserved.

**Requirements**: ATLAS-01, ATLAS-02, ATLAS-03, ATLAS-04

**Background**: SEED-004 documents the gap. spine-ts 4.2 already exposes `region.rotate` (boolean) on `TextureAtlasRegion` — parsed during atlas-text reading and surfaced for renderer-level UV swap in spine-ts's GPU pipeline. Our pipeline doesn't use spine-ts rendering (we read region geometry directly), so the rotation flag was never consumed; the `RotatedRegionUnsupportedError` hard-throw at `src/core/errors.ts:154` is the v1.0-era stop-gap. The fix has three coordinated edits:
- **Loader** — drop the `RotatedRegionUnsupportedError` throw site; rotated regions flow through `analyzer.ts` carrying their `rotate: true` flag like any other geometry datum.
- **Bounds (`src/core/bounds.ts`)** — `attachmentWorldAABB` (or whichever function reads the region's source W×H to compute the world-space AABB) swaps W↔H when `region.rotate === true` BEFORE feeding into the world-transform. This matches what spine-ts does at GPU time: the packed image is rotated 90° in atlas space, but at render time it occupies its unrotated WxH footprint.
- **ExportPlan output dims** — for rotated regions, the exported per-region PNG dimensions reflect the unrotated W×H. Animators downstream-bake these as if the region were never rotated; the rotation is purely a packer-density optimization.

The packer-options interaction is locked by memory `project_atlas_pack_options_atlas_source_only.md`: atlas-source mode handles strip-whitespace fully (atlas-orig + isotropic correction); rotation in atlas-source mode currently hard-fails by design — this phase replaces that hard-fail with full support. Atlas-less mode is unaffected by both packer options. The fixture for ATLAS-04 must be a re-pack of an EXISTING in-repo fixture (e.g. SIMPLE_PROJECT or a stripped derivative), NOT one of the gitignored proprietary rigs (Chicken/Girl/Jokerman). Per memory `feedback_gitignore_fixtures_check_test_refs.md`: before committing any new fixture path, grep `tests/` and `src/` to confirm nothing references it under a soon-to-be-gitignored pattern.

**Constraints to preserve**:
- **Layer 3 invariant** — bounds.ts edits stay in pure-TS `src/core/`; no DOM/electron/sharp imports. Loader edit is in `src/core/loader.ts`. ExportPlan dim swap is in the export-plan builder (likely `src/main/`); the ExportPlan IPC contract continues to carry plain numbers, not "rotated/unrotated" discriminators (the swap happens upstream of the IPC envelope).
- **Atlas-less mode untouched** — `src/core/synthetic-atlas.ts` continues to emit `rotated:false` always (each PNG is its own region; rotation is a packer-only concern). Out-of-Scope per REQUIREMENTS.md.
- **Strict loaderMode separation** preserved (memory `project_strict_loadermode_separation.md`) — the rotation handling lives entirely on the atlas-source code path; `load.atlasPath` continues to gate every read of the opposite artifact set.
- **Uniform-only export math** preserved (memory `project_phase6_default_scaling.md`) — the W↔H swap is a pre-export normalization that produces unrotated W×H; the downstream uniform-isotropic export math (Phase 6 LOCKED) operates on the unrotated dims and remains unchanged.
- **Sampler visibility invariant** preserved (memory `project_sampler_visibility_invariant.md`) — sampler measures all skin-declared attachments verbatim; rotation handling is strictly downstream in bounds + export-plan. Sampler code path untouched.
- **Peak-anchored override invariants** preserved (memory `project_peak_anchored_invariants.md`) — `applyOverride` remains canonical-relative; no source-ratio adjustment downstream. Rotated regions feed the peak-anchored math the same canonical W×H (unrotated) that the renderer would see.
- **`computeExportDims` canonical base** preserved (memory `project_compute_export_dims_canonical_base.md`) — `outW` and `sourceRatio` continue to use `canonicalW` (the unrotated W after the swap), not the packed-rotated dim.
- **Existing `RotatedRegionUnsupportedError` class removal must clean up all references** — call sites in loader/analyzer + tests + the error-envelope discriminator union (D-158/D-171) must drop the case in lockstep. No dangling references to a removed error type.
- **Fixture is in-repo and small** — re-pack of SIMPLE_PROJECT or similar (NOT Chicken/Girl/Jokerman, all of which are gitignored proprietary). Verify no `.gitignore` rule shadows the new fixture path before committing. (memory `feedback_gitignore_fixtures_check_test_refs.md`)

**Success Criteria** (what must be TRUE):
  1. User loads an atlas-source project whose `.atlas` contains regions packed with `rotate: true`; the loader accepts the project and populates the Global + Animation Breakdown panels normally. The `RotatedRegionUnsupportedError` "Rotated atlas regions are not supported" error message no longer appears for rotated atlases. (ATLAS-01)
  2. For rotated regions, the world-space AABB computed by `attachmentWorldAABB` in `src/core/bounds.ts` matches what the spine-ts runtime would render at identity scale — verified by a unit test that asserts the rotated-region AABB equals the unrotated-region AABB for a region pre-and-post the Spine packer's rotation toggle. (ATLAS-02)
  3. User clicks Optimize Assets on a project with rotated regions; the exported per-region PNG dimensions match the unrotated (visually-correct) W×H, NOT the packed-rotated dims. Verified by inspecting an exported PNG's actual pixel dimensions vs the source pre-rotation dims. (ATLAS-03)
...
**Wave 2** *(blocked on Wave 1 completion)*
- [x] 33-03-PLAN.md — Lockstep removal of `RotatedRegionUnsupportedError` + `'rotated-region-unsupported'` ExportError kind across 9 source/test sites in ONE atomic commit (D-158/D-171 precedent); ATLAS-01

**Wave 3** *(blocked on Wave 2 completion)*
- [x] 33-04-PLAN.md — D-01 loader walk (post-`readSkeletonData` SWAP form per RESEARCH §"D-01 Formula Derivation") + ATLAS-01 fixture-driven test + ATLAS-02 16-case bone-rotation AABB matrix; ATLAS-01, ATLAS-02
...
### Phase 34: File > Open menu accepts Spine skeleton JSON files (not only `.stmproj`)

**Goal**: After this phase, the File → Open menu item (and its `Cmd+O` / `Ctrl+O` accelerator) accepts both `.stmproj` project archives AND raw Spine skeleton `.json` files. Selecting a `.json` routes through the same loader entry as the drag-drop surface, preserving strict loaderMode separation (atlas-source vs atlas-less detected from sibling artifacts in the JSON's folder). Keyboard-driven workflows reach full parity with drag-drop.

**Depends on**: Phase 08.1 (locate-skeleton recovery + dirty-guard wiring, already complete) and Phase 08.2 (File menu + `Cmd+O` accelerator gating fix, already complete). Reuses the existing drag-drop loader entry in `src/core/loader.ts`. No new IPC channels expected.
...
**Background**: Phase 08.2 wired File → Open to `.stmproj` only. Drag-drop has always accepted both `.stmproj` and skeleton `.json` (with atlas-source vs atlas-less detected from sibling artifacts). This phase closes that asymmetry so keyboard-driven users — and users with admin-elevated Windows shells where drag-drop is disabled per PLATFORM-01 — can open skeleton JSON the same way.

**Constraints to preserve**:
- **Strict loaderMode separation** (memory `project_strict_loadermode_separation.md`) — `.json` selection must route through the same atlas-source vs atlas-less detection as drag-drop. No new code path that re-derives loaderMode from a different signal.
- **Dirty-guard** (Phase 08.1) — opening a `.json` over an in-progress unsaved project must trigger the existing "Save changes?" confirmation flow.
- **Windows admin DnD fallback** (PLATFORM-01) — this phase strengthens the documented "use File → Open instead" advisory by making it true for skeleton JSON, not only `.stmproj`.

**Success Criteria** (what must be TRUE):
  1. File → Open dialog filter accepts both `.stmproj` and `.json` extensions on macOS + Windows.
  2. Selecting an atlas-source skeleton JSON loads the project end-to-end identically to drag-dropping the same file (Global panel, Animation Breakdown, atlas detection).
  3. Selecting an atlas-less skeleton JSON (images folder, no `.atlas`) loads identically to its drag-drop equivalent.
...
### Phase 35: Region-keyed export plan — propagate Phase 29 dedup to Optimize modal + Atlas Preview optimized mode

**Goal**: After this phase, the Optimize Assets modal and the Atlas Preview optimized-mode tile expansion produce **one entry per atlas region** on multi-skin atlas-source projects (matching the Global table count and the cardinality of `summary.regions`), not one entry per unique authored attachment name. On the `fixtures/SKINS/JOKERMAN_SPINE.json` regression fixture (7 skins, 160 regions), the Optimize modal header reads `Optimize Assets — 160 images` (rather than today's 23), and Atlas Preview optimized mode emits one preview tile per region. The fix is keyed on the Phase 29 region-dedup contract: `buildExportPlan` iterates `summary.regions` (RegionRow[]) instead of `summary.peaks`. Lockstep parity between `src/core/export.ts` and `src/renderer/src/lib/export-view.ts` is preserved by mirroring the change byte-identically. Atlas-less path is unaffected (`analyzeRegions` is loaderMode-agnostic).

**Depends on**: Phase 29 (region-keyed dedup contract — `summary.regions` already populated end-to-end and threaded over IPC; consumed today by the Global table). No new IPC channels expected.

**Requirements**: DEDUP-04, DEDUP-05, DEDUP-06 (added 2026-05-12 via `/gsd-plan-phase 35` — extends Phase 29's DEDUP-* family; see `.planning/REQUIREMENTS.md` § Region-Keyed Export Plan).

**Background**: Phase 29 introduced region-keyed dedup to fix path-indirected duplicate rows on the Global table surface. The Phase 29 fix was scoped to `analyzer.ts → summary.regions → Global table`. Three sibling surfaces — Optimize Assets modal, Atlas Preview optimized mode, and `buildExportPlan` itself — still consume `summary.peaks` (the older attachment-name-deduped output of `analyze()`), so they collapse skin-namespaced regions that share a base name. This was acknowledged at v1.3.1 close in `.planning/STATE.md` Deferred Items: `debug | path-indirected-duplicate-rows | pending_phase_plan — root cause closed by Phase 29 region-dedup fix; debug doc kept as a v1.4 reference for related surfaces (Atlas Preview optimized-mode tile expansion)`. Surfaced on 2026-05-12 by user testing of the new `fixtures/SKINS/` multi-skin fixture (`.planning/debug/skins-optimize-undercount.md` — diagnosis complete, fix not applied).

**Constraints to preserve**:
- **Region-keyed dedup contract** (Phase 29) — every export-plan consumer must key on `regionName`, never on `attachmentName` alone. Includes overrides keyed by `regionName` per Phase 29 D-04 (already in place; iteration source is the gap).
- **Lockstep parity** (`src/core/export.ts` ↔ `src/renderer/src/lib/export-view.ts`) — these two `buildExportPlan` copies are byte-identical by contract; the migration to `summary.regions` must land in both files in the same commit, with structurally identical loop bodies.
- **Strict loaderMode separation** (memory `project_strict_loadermode_separation.md`) — `analyzeRegions` is mode-agnostic; the fix must not introduce an atlas-source-only or atlas-less-only code path. Atlas-less correctness must be verified with an existing atlas-less fixture.
- **Uniform export sizing** (memory `project_phase6_default_scaling.md`) — both axes scale by the same factor in the export plan rows; switching the iteration source must not perturb output dims for regions that have explicit overrides (the override resolution must still key on `regionName`).
- **`computeExportDims` uses `canonicalW` as the `outW` base** (memory `project_compute_export_dims_canonical_base.md`) — the per-region path must preserve canonical-vs-source-vs-page dim invariants for sequence/atlas-source-drift cases.
- **Peak-anchored override invariants** (memory `project_peak_anchored_invariants.md`) — the three invariants from the 2026-05-05 redesign must hold per region after the iteration source swap.

**Success Criteria** (what must be TRUE):
  1. Loading `fixtures/SKINS/JOKERMAN_SPINE.json` and clicking **Optimize Assets** shows a modal header of `Optimize Assets — 160 images` (= `summary.regions.length`).
  2. The modal body lists each skin-namespaced region as a distinct row (e.g. `AVATAR/CARDS_L_HAND_1`, `BUSINESS/CARDS_L_HAND_1`, `IRONMAN/CARDS_L_HAND_1`, `JOKER/CARDS_L_HAND_1` all appear).
  3. Atlas Preview optimized-mode tile expansion emits one tile per region (one `AtlasPreviewInput` per `ExportRow`, count matches `summary.regions.length`).
  4. `buildExportPlan` (and its renderer parity copy `export-view.ts`) iterates `summary.regions` — verifiable by grep at both file loci.
  5. Existing fixtures (`fixtures/SIMPLE_PROJECT/`, `fixtures/Rotated/`, atlas-less fixtures) continue to produce identical export plans (regression coverage — no behavior change on single-skin or attachment-name-unique inputs).
  6. Override resolution per region still keyed by `regionName` (Phase 29 D-04 preserved).
...
**Wave 2** *(blocked on Wave 1 completion)*
- [x] 35-03-atlas-preview-consumer-audit-PLAN.md — Static audit of `atlas-preview-view.ts` + `atlas-preview.ts` optimized-mode tile expansion (expected no-op) + manual UAT loading fixtures/SKINS/JOKERMAN_SPINE.json to verify success criteria 1/2/3.
- [x] 35-04-regression-tests-multi-skin-PLAN.md — Add Phase 35 describe block to tests/core/export.spec.ts with 3 synthetic-fixture tests (multi-skin, per-region override, single-skin backward-compat) + 1 fixture-driven integration test loading fixtures/SKINS/JOKERMAN_SPINE.json.

**UI hint**: no (mathematical/contract change; visible only via row count and per-region rows in two existing UI surfaces)
...
_(Empty — items 999.4–999.9 promoted into v1.3 Phases 24/25/26.1/28 and removed 2026-05-06.)_
