...
- **D-07:** PERF-01's measurement subject is an **owner-authored, redistributable, in-repo complex 4.3 rig** (user-selected). It must be original (no licensed third-party assets like `fixtures/Girl/`) so it can be committed and the budget gate can run on **CI** — strictly stronger than the existing Girl gate's `it.skipIf(process.env.CI)` local-only pattern.
  - **DELIVERED + VERSION-ALIGNED 2026-05-18 (post-discussion):** `fixtures/spineboy_4.3/` — the official Esoteric **Spine Boy** example (redistributable; ships in every spine-runtimes example repo). The PERF-01 subject is the canonical triple **`fixtures/spineboy_4.3/spineboy-pro.{json,atlas,png}`** (67 bones, 52 slots, 11 animations, 1 skin, **14 constraints**, 255 KB JSON). The owner re-exported from the stable editor build that produced the Phase-44 fixtures; the dir was cleaned to exactly this one triple (+ a gitignored `.DS_Store`) — no ambiguous extra variants, `.skel` binaries, or `-ess`/`-pma`/`-run` files remain. `git check-ignore`: all three are **committable / NOT gitignored** → CI-enforceable (D-07 satisfied).
  - **VERSION-ALIGN RE-EXPORT — DONE & ACCEPTED 2026-05-18:** the first delivery carried `spine: "4.3.75-beta"` — an outlier vs the pinned `spine-core@4.3.0` runtime and every other in-repo 4.3 fixture (`SIMPLE_PROJECT_43`/`XTRA-02`=`4.3.01`, `XTRA-01`/`SLIDER-01`=`4.3.02`). The owner re-exported `spineboy-pro` at the aligned editor build. **Verified landed: `skeleton.spine` = `4.3.01`** (no `-beta`), structure preserved **identically** to the beta (67/52/11/14 — nothing dropped). This places the perf rig on the exact parse path Phase-44 ORCL-02 already proved correct/complete for `4.3.0x` tokens, dissolving the D-11 false-green-budget risk **by construction**. (Token verified, not assumed — memory [[project_spine_4_3_editor_atlas_format_editor_driven]].)
...
- **D-11 (RESOLVED 2026-05-18 — no longer a researcher action):** the original `4.3.75-beta`/`4.3.0`-parser fidelity risk is **eliminated by construction**. The owner re-exported `spineboy-pro` at `spine 4.3.01` (verified token, no `-beta`; structure byte-for-byte equivalent in counts: 67/52/11/14) — the same proven-good parse path as the Phase-44 ORCL-02-gated `4.3.0x` fixtures. The acceptance check is **already PASSED** (token aligned + parse-count sane). The researcher need only re-confirm the token in passing during the normal load; no parse-fidelity investigation, no fallback path needed (the `4.3.75-beta` JSON was removed from the fixture dir).
...
- **D-10:** The bundled owner-export pass for Phase 46 is now **two small actions** (down from a from-scratch complex-rig authoring): (a) **version-align re-export of `spineboy-pro`** at the `4.3.0x` editor token, replacing the beta JSON+atlas+png (D-07); (b) **`fixtures/SLIDER_4_3/NOTES.txt`** — the Spine-4.3-editor-observed slider peak (D-03). The researcher/planner authors one precise §-spec covering both (export-version target + acceptance token for (a); value/frame/units to read for (b)), as `42-OWNER-EXPORT-SPEC.md` did for Phase 44. No from-scratch rig authoring remains on the critical path.
...
- `fixtures/spineboy_4.3/spineboy-pro.{json,atlas,png}` — the **delivered + version-aligned** PERF-01 complex 4.3 rig (D-07; spine **`4.3.01`**, 67 bones / 52 slots / 11 anims / 14 constraints, committable/not-gitignored). D-11 risk RESOLVED by construction (aligned to the Phase-44-proven `4.3.0x` parse path).
