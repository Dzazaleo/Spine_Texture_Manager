...
- [ ] `tests/core/ipc.spec.ts` — forces src/shared/types.ts + src/main/ipc.ts to exist; covers F1-integrated + D-10 error envelope
- [ ] `tests/core/summary.spec.ts` — forces src/main/summary.ts to exist; covers D-21 shape + D-22 structuredClone + D-16 sort order
- [ ] `tests/arch.spec.ts` — three-layer-defense Layer 3 (renderer↛core grep) + D-23 portability grep
